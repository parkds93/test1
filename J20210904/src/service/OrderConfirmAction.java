...
public class OrderConfirmAction implements CommandProcess {
	...
	public String requestPro(HttpServletRequest request, HttpServletResponse response)
		...
		//가지고 올 정보 상품에서 ()
		// 주문에서 (구매일,주문상태)	
		// 주문상세에서 (물품별 수량,가격(같은주문번호모두합산))
		//아이디를 파라미터로 해당 아이디의 주문 내역을 표시한다.
		...
		try {
			...
			//id를 request에서 가져옴
			int totCnt = od.getTotalCntId(id);
			...
			List<Orders> list = od.OrderCf(startRow, endRow, id);	
			//bd라는 Ordersdao의 싱글턴 객체의 OrderCf메서드를 호출한다. 시작줄번호, 끝줄 번호를 파라미터로 가져간다.
